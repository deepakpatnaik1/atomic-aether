...
//
//  ATOM 202: LLM Services - OpenAI provider
//
...
class OpenAIService: LLMService {
    ...
    func sendMessage(_ request: LLMRequest) async throws -> AsyncThrowingStream<LLMResponse, Error> {
        ...
        // Add response_format if present in metadata
        if let responseFormat = request.metadata?["response_format"] as? [String: Any] {
                    ...
                    if isStreaming {
                        ...
                        if let json = try? JSONSerialization.jsonObject(with: responseData) as? [String: Any],
                           ...
                           let content = message["content"] as? String {
                            ...
                            // Send the full JSON content
                            continuation.yield(.content(content))
                            continuation.yield(.done)
                        } else {
                            throw LLMError.invalidResponse("Failed to parse OpenAI response")
                        }
                    }
